...
async function loadUsersAndLikes() {
    ...
    try {
        if (auth.currentUser) {
            ...
            // Load only public decks
            const publicDecksSnapshot = await firestoreOperationWithRetry(() => db.collection('publicDecks').get());
            users = [];
            publicDecksSnapshot.forEach(doc => {
                const deckData = doc.data();
                const userData = {
                    uid: 'public',
                    username: deckData.creator,
                    decks: [{
                        name: deckData.name,
                        heroId: deckData.heroId,
                        troops: deckData.troops,
                        description: deckData.description,
                        isPublic: deckData.isPublic,
                        creator: deckData.creator,
                        likes: deckData.likes || 0
                    }]
                };
                users.push(userData);
            });
            console.log("Public decks loaded for unauthenticated user:", users);

            // Load userLikes for public decks
            const likesSnapshot = await firestoreOperationWithRetry(() => db.collection('userLikes').get());
            userLikes = {};
            likesSnapshot.forEach(doc => {
                userLikes[doc.id] = doc.data().value;
...
async function syncPublicDeck(deck, action = 'add') {
    if (!deck.isPublic && action === 'add') return;
    try {
        const deckData = {
            name: deck.name,
            heroId: deck.heroId,
        ...
        if (action === 'add') {
            await firestoreOperationWithRetry(() => 
                db.collection('publicDecks').doc(`${deck.creator}:${deck.name}`).set(deckData)
            );
        } else if (action === 'remove') {
            await firestoreOperationWithRetry(() => 
                db.collection('publicDecks').doc(`${deck.creator}:${deck.name}`).delete()
            );
        }
    } catch (error) {
        console.error(`Error syncing public deck (${action}):`, error);
        throw error;
    }
}
...
authForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const username = DOMPurify.sanitize(document.getElementById('username-input').value);
    const password = document.getElementById('password-input').value; // No sanitizar contraseña, pero sí hashearla
        ...
        deckCard.querySelector('.toggle-public-btn').addEventListener('click', async () => {
            try {
                ...
                if (!deck.isPublic) {
                    ...
                    await syncPublicDeck(deck, 'remove');
                } else {
                    await syncPublicDeck(deck, 'add');
                }
                await firestoreOperationWithRetry(() => db.collection('users').doc(currentUser.uid).update({ decks: currentUser.decks }));
                await loadUsersAndLikes();
                displayUserDecks();
...
function showDeckDetails(deck) {
    deckDetailsTitle.textContent = DOMPurify.sanitize(deck.name);
    deckDetailsCreator.textContent = DOMPurify.sanitize(deck.creator || currentUser.username);
    deckDetailsDescription.textContent = DOMPurify.sanitize(deck.description || 'No description provided.');
    const hero = heroes.find(h => h.id === deck.heroId);
    deckDetailsHero.textContent = DOMPurify.sanitize(hero ? hero.name : 'Unknown');
    deckDetailsHeroImage.innerHTML = `<img src="${getImageUrl(hero ? hero.name : 'Unknown', 'heroes')}" alt="${DOMPurify.sanitize(hero ? hero.name : 'Unknown')}" onerror="this.src='https://via.placeholder.com/100?text=Hero+Image+Not+Found';">`;

    deckDetailsTroops.innerHTML = '';
    deck.troops.forEach(troopId => {
        const troop = troops.find(t => t.id === troopId);
        const troopName = DOMPurify.sanitize(troop ? troop.name : 'Unknown');
...
function showDeckModal(mode, deck = null) {
    ...
    deckForm.onsubmit = async (e) => {
        e.preventDefault();
        const deckName = DOMPurify.sanitize(deckNameInput.value);
        const heroId = parseInt(heroSelect.value);
        const troops = troopSelects.map(s => parseInt(s.value));
        const description = DOMPurify.sanitize(deckDescriptionInput.value);
        ...
        try {
            ...
            await firestoreOperationWithRetry(() => db.collection('users').doc(currentUser.uid).update({ decks: currentUser.decks }));
            if (newDeck.isPublic) {
                await syncPublicDeck(newDeck, 'add');
            } else {
                await syncPublicDeck(newDeck, 'remove');
            }
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    ...
    navigator.serviceWorker.register('/WonderDecks/sw.js', { scope: '/WonderDecks/' }) // Ajustar la ruta y el ámbito
      .then(registration => {
